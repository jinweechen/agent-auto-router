...
Prefer `aar.ps1` for ordinary signed-in local CLI use. Its `standard` profile selects `balance + workspace-write` limited to the explicit workdir, adaptively inspects the repository only for code/path/debugging tasks, loads the configured learning policy, writes privacy-minimized feedback, reports orchestration recommendations without executing them, and reuses the selected model only within the current run. `-NoFeedback` suppresses persistence for one standard run without disabling policy loading. `safe` selects `balance + read-only` with repository inspection, learning, feedback, and affinity off. Neither profile starts multi-agent execution. Do not use this convenience wrapper to synthesize Desktop host metadata. Use the full Desktop workflow whenever execution is delegated by a host rather than explicitly started by the user.
...
9. After every attempted stage instance, have the trusted host produce one closed `agent-auto-router.execution-receipt` from the exact executable Desktop plan and stage. Preserve its canonical `planBinding`, use a stable `attemptBindingId` for that plan attempt, and compute `receiptId` from the complete receipt content. Record the plan's requested and resolved model/effort separately from the host-observed actual identity. If host telemetry cannot prove the actual identity, record `actual.state=unresolved`; never copy the resolved identity into `actual` as a guess.
10. Keep completion, host-UI reconciliation, and acceptance separate. Order deadline and terminal observations with trusted monotonic `deadlineSequence` and `terminalSequence` values: a terminal observed first wins, while a later terminal never erases an earlier timeout. Authoritative terminal evidence may complete a child while advisory UI still says `processing`; record `stale_host_ui` without relaunching it. A successful terminal outcome is not accepted until actual identity matches and every required check has trusted evidence. When `wouldWrite=true`, acceptance additionally requires trusted, content-digested workspace evidence with `state=changed` under the `changed-required` policy. An `agent-claim` can never accept its own work.
11. When router-managed execution was explicitly requested, keep at most one open Desktop run in a parent turn and reconcile all receipts before the final response.
12. Submit an execution report only when learning submission was explicitly enabled for that run. The execution receipt is operational evidence and is not a learning label. Never add task text or child output. Do not submit DryRun plans.
...
The Desktop protocol supports A-F. A/E/F remain direct; B/C/D use a staged multi-agent DAG with bounded workers and exactly one final writer. Orchestration is an advanced, explicitly requested router workflow; ordinary host tasks never enter it merely because the Skill is installed. Reduce a high-risk worker route to a direct plan plus a confirmation-required recommendation; clear that gate only when the caller explicitly supplies `ConfirmHighRiskOrchestration`. The selected direct model must be available exactly. Plan-time role resolution reports `preferredModel`, resolved `model`, and `modelResolution`; the post-attempt receipt separately records host-observed actual identity or an explicit unresolved state. Never downgrade, cross backends, hide a change, or claim observation without host telemetry. A host lacking per-child `workdir` or `sandbox` arguments may execute only plans whose exact boundary is already inherited; otherwise block rather than relying on prompt-only isolation. Workspace snapshots and change attribution are host concerns and are not router launch gates. Missing trusted metadata, an out-of-root workdir, unresolved role models, insufficient call budget, validation escalation, and non-default CLI context mode block before launch. `-DryRun` returns the same non-executable plan with `plannedAgentCalls=0`.
...
- Use default `-ModelAffinity session` to prefer the selected model for compatible roles within one run without reading or writing state. Use `sticky` only when a trusted host supplies a lowercase HMAC-SHA256 conversation-key digest and the current pinned trusted model, plus optional pinned effort and host-observed residency state. Keep a compatible pin by default; upgrade immediately when the model or effort is weaker, and replace it when the exact trusted runtime availability snapshot says it is unavailable. Downgrade only after at least three pin turns, a 600-second switch cooldown, an explicit checkpoint, and explicit downgrade confirmation. Return `pinUpdateRequired`, `pinUpdateModel`, and `pinUpdateEffort` for the host to carry; never persist the pin state in feedback. Enable `auto` explicitly for workspace-evidence reuse: consider only successful same-workspace/same-strategy evidence within 30 minutes; allow same-tier reuse directly, while retaining one stronger tier requires at least 15% cached-input read ratio. Cache writes are rebuild cost and never increase that signal. Never retain a weaker model or jump more than one tier in workspace-evidence mode. After three usable samples below 5% cache reads, return role assignment to profile-exact behavior. Use `off` for exact profile role assignments. Token ratios are not quality labels or billing estimates, and without provider billing or subscription-credit inputs do not claim monetary savings.
...
- Never treat requested or resolved model identity as host-observed actual identity. Keep actual identity unresolved when telemetry is absent.
- Never treat an advisory UI label, a child self-report, or process exit success alone as accepted completion.
