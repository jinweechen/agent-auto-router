...
- Use default `-ModelAffinity session` to prefer the selected model for compatible roles within one run without reading or writing state. Use `sticky` only when a trusted host supplies both a lowercase HMAC-SHA256 conversation-key digest and the current pinned trusted model; retain it while it satisfies backend, capability, and tier floors, and return `pinUpdateRequired=true` when the current task must upgrade. Enable `auto` explicitly for workspace-evidence reuse: consider only successful same-workspace/same-strategy evidence within 30 minutes; allow same-tier reuse directly, while retaining one stronger tier requires at least 15% cached-input read ratio. Cache writes are rebuild cost and never increase that signal. Never retain a weaker model or jump more than one tier in workspace-evidence mode. After three usable samples below 5% cache reads, return role assignment to profile-exact behavior. Use `off` for exact profile role assignments. Token ratios are not quality labels or billing estimates.
