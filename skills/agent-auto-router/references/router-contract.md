...
`ExecutionBackend=desktop` emits one `agent-auto-router.desktop-plan.v2` JSON object; it does not execute a process. The caller must declare the exact model IDs currently supported by its Desktop `spawn_agent` runtime and the trusted current-turn permission snapshot. A ready plan contains the selected model, effort, normalized workdir, context profile, route identity, effective permissions, and `forkTurns=none`, but omits task text. `routingModelCalls` counts only local routing calls and is always zero; `plannedAgentCalls` is one only for an executable ready plan.
...
Desktop v2 supports only direct A/E/F topology and the Codex backend. It accepts inherited `read-only`, `workspace-write`, and `danger-full-access`; the latter is usable only when it is the current host sandbox. If routing selects B/C/D, the selected model belongs to a foreign backend, the model is unavailable, permission metadata is missing, or the workdir exceeds the host boundary, return a structured blocked plan with zero planned calls and launch nothing. Reject CLI-only feedback destinations, validation escalation, and non-default CLI context mode. Never silently change model, effort, tier, provider, backend, or permissions.
